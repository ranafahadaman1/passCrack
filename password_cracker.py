...
def detect_hash_algorithm(hash_string):
    """
    Detect the hash algorithm based on the length of the hash string.
    Returns the algorithm name or None if not recognized.
    """
...
Examples:
  ...
  2. Crack a hash using a dictionary (algorithm auto-detected):
     python password_cracker.py crack -H "..." -d wordlist.txt

  3. Crack a hash using brute-force (algorithm auto-detected):
     python password_cracker.py crack -H "..." -b -c "abc" -l 5
""",
    )
    ...
    hash_parser = subparsers.add_parser("hash", help="Hash a plaintext password.")
    hash_parser.add_argument(
        "-p", "--password", required=True, help="The plaintext password to hash."
    )
    hash_parser.add_argument(
        "-a",
        "--algorithm",
        default=DEFAULT_HASH_ALGORITHM,
        help=f"Hash algorithm (default: {DEFAULT_HASH_ALGORITHM}). E.g., md5, sha1, sha256.",
    )

    # Parser for the crack command
    crack_parser = subparsers.add_parser("crack", help="Crack a given password hash.")
    ...
    attack_group.add_argument(
        "-b", "--brute-force", action="store_true", help="Enable brute-force attack."
    )
    ...
    if args.command == "hash":
        ...
        if args.algorithm not in ALLOWED_HASH_ALGORITHMS:
            print(
                f"Error: Unsupported hash algorithm '{args.algorithm}'. Allowed: {ALLOWED_HASH_ALGORITHMS}"
            )
            sys.exit(1)
        ...
        if args.algorithm:
            ...
            if not algorithm:
                print(
                    f"Error: Could not detect hash algorithm from hash length ({len(args.target_hash)} characters)."
                )
                print(f"Supported lengths: 32 (MD5), 40 (SHA-1), 64 (SHA-256)")
                print(
                    "Please specify the algorithm with -a or provide a supported hash."
                )
                sys.exit(1)
            print(f"Probable hash algorithm: {algorithm}")

        if args.wordlist_path:
            # Do dictionary attack
            dictionary_attack(
                args.target_hash, algorithm, args.wordlist_path, args.processes
            )
        elif args.brute_force:
            # Check if length is too long
            if args.max_length > MAX_BRUTE_FORCE_LENGTH:
                print(
                    f"Warning: Max length {args.max_length} exceeds the configured limit of {MAX_BRUTE_FORCE_LENGTH}."
                )
