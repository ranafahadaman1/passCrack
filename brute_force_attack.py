...
def check_brute_force_task(prefix, charset, length_to_gen, target_hash):
    ...
    if length_to_gen == 0:
        if multiprocessing_utils.found_password_event.is_set():
            return None
        if (
            multiprocessing_utils.hash_func(prefix.encode("utf-8")).hexdigest()
            == target_hash
        ):
...
def brute_force_attack(target_hash, algorithm, charset, max_length, num_processes):
    ...
    total_combinations = sum(charset_len**length for length in range(1, max_length + 1))
        ...
        with Pool(
            ...
            for length in range(1, max_length + 1):
                ...
                if length == 1:
                    ...
                    tasks = [
                        (char, charset, length - 1, target_hash) for char in charset
                    ]
